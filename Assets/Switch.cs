...
public class Switch : NetworkBehaviour
{
    ...
    // NetworkVariable 선언을 해보세요. 유저가 충돌 이벤트를 했을 때 작동하고 잇는지 안하는지 체크하는 오브젝트
    // bool IsSwitchOn
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        // 생성된 객체가 서버 컴퓨터안에서 충돌했을 때 발생하는 이벤트.
    ...
    private void FixedUpdate() // 물리적 계산을 처리할 때 호출하면 좋다. TriggerEvent 발생 후 에 실행이 보장된다.
    {
        if (!IsSpawned) { return; }
